...
public sealed class BanManager : IBanManager, IPostInjectInit
{
    ...
    public async void CreateServerBan(NetUserId? target, string? targetUsername, NetUserId? banningAdmin, (IPAddress, int)? addressRange, ImmutableArray<byte>? hwid, uint? minutes, NoteSeverity severity, string reason)
    {
        ...
        var targetNameRP = target is null ? "null" : $"{targetUsername}";
        ...
        if (banningAdmin is not null)
        {
            var publicLogMessage = Loc.GetString(
                expires is not null ? "server-ban-string-rp" : "server-ban-string-rp-perm",
                ("admin", adminName),
                ("expires", expiresString),
                ("name", targetNameRP),
                ("reason", reason));

            _chat.DispatchServerAnnouncement(publicLogMessage, Color.Azure);
        }
    ...
    public async void CreateRoleBan(NetUserId? target, string? targetUsername, NetUserId? banningAdmin, (IPAddress, int)? addressRange, ImmutableArray<byte>? hwid, string role, uint? minutes, NoteSeverity severity, string reason, DateTimeOffset timeOfBan)
    {
        ...
        var roleBanMessage = Loc.GetString(
            "cmd-roleban-success",
        ...
        var roleBanMessageRP = Loc.GetString(
            expires is not null ? "cmd-roleban-success-rp" : "cmd-roleban-success-rp-perm",
            ("target", targetUsername ?? "null"),
            ("role", role), ("reason", reason),
            ("length", length)
        );
        _chat.SendAdminAlert(roleBanMessage);
        _chat.DispatchServerAnnouncement(roleBanMessage, Color.Azure);
