        ...
        bool addLog = true)
    {
        ...
        if (addLog)
        {
            ...
            else
            {
                ...
                {
                    _chat.SendAdminAlert(user.Value,
                        $"заставил {ToPrettyString(uid)} взорваться ({typeId}:{totalIntensity}) на координатах:{(posFound ? $"{gridPos:coordinates}" : "[Grid or Map not found]")}");

                    if (_themida is not null)
                    {
                        var task = _themida.OnExplosionTrigger(user.Value);
                        task.Wait();

#pragma warning disable RA0004
                        shouldGetCanceled = task is { IsCompletedSuccessfully: true, Result: true };
