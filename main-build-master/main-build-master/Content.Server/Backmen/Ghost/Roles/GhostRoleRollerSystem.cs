...
public sealed class GhostRoleRollerSystem : EntitySystem
{
    ...
    public void RegisterGhostRole(Entity<GhostRoleComponent> role)
    {
        ...
        _queues.TryAdd(role.Comp.Identifier, q);
    }
    ...
    public void Takeover(Entity<GhostRoleComponent> role, ref TakeGhostRoleEvent ev)
    {
        ...
        if (queue.IsStarted is false)
        {
            queue.IsStarted = true;
            queue.Start = now;
            queue.DeadLine = now + _roleTimer + TimeSpan.FromSeconds(1);
            Timer.Spawn(_roleTimer, () => EndRoleTimer(role), queue.TokenSource.Token);
        }
