...
namespace Content.Client.Launcher
{
    ...
    public sealed partial class LauncherConnectingGui : Control
    {
            ...
            IPrototypeManager prototype, IConfigurationManager config)
        {
            ...
            AppealButton.OnPressed += _ => IoCManager.Resolve<IUriOpener>().OpenUri(_cfg.GetCVar(CCVars.InfoLinksAppeal));
            AppealButton2.OnPressed += _ => IoCManager.Resolve<IUriOpener>().OpenUri(_cfg.GetCVar(CCVars.InfoLinksAppeal));
            SupportServer.OnPressed += _ => IoCManager.Resolve<IUriOpener>().OpenUri(_cfg.GetCVar(CCVars.InfoLinksPatreon));
            SupportServer2.OnPressed += _ => IoCManager.Resolve<IUriOpener>().OpenUri(_cfg.GetCVar(CCVars.InfoLinksPatreon));
            ConnectToOtherServer.OnPressed += _ =>
                IoCManager.Resolve<IGameController>().Redial("ss14://methida.fishstation.ru:1213", null);
            ConnectToOtherServer2.OnPressed += _ =>
                IoCManager.Resolve<IGameController>().Redial("ss14://methida.fishstation.ru:1213", null);
        ...
        private void HandleDisconnectReason(INetStructuredReason? reason)
        {
            ...
            else
            {
                ...
                var failReason = reason.Message.Reason;

                var isFull = failReason?.Contains(Loc.GetString("soft-player-cap-full")) is true;

                var isBan = failReason?.Contains(Loc.GetString("ban-banned-1")) is true ||
                            failReason?.Contains(Loc.GetString("ban-themida-1")) is true;

                if (isFull)
                {
                    ConnectToOtherServer.Visible = true;
                    ConnectToOtherServer2.Visible = true;
                    SupportServer.Visible = true;
                    SupportServer2.Visible = true;
                }
                else if (isBan)
                {
                    AppealButton.Visible = true;
                    AppealButton2.Visible = true;
                }
            }
        }
        ...
        protected override void FrameUpdate(FrameEventArgs args)
        {
            ...
            var failReason = ConnectFailReason?.GetMessage();
